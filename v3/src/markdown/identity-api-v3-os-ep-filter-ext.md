...
Endpoint Filter provides the ability to create ad-hoc catalogs for each
project-scoped token request by associating service endpoints with projects
either statically through project-endpoint associations or dynamically via
...
the same characteristics (e.g. service, type, region).
...
interface=”admin" and service_id= "2942f5baccdf4384b5d58adfd3de86d2"
...
Create an Endpoint Group Filter.
...
Returns a specific endpoint group.
...
Updates a specific endpoint group.
...
Remove a specific endpoint group.
...
Response:
  ...
  Furthemore, we can pass in query parameters ``project_id`` to further filter
  the list of endpoint groups. For example:

  - List Endpoint Groups Associated with Project:

      ``GET /OS-EP-FILTER/endpoint_groups?project_id={project_id}``

### Project to Endpoint Group Relationship CRUD
