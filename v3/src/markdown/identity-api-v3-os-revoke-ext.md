...
- `audit_chain_id` (string)
   ...
   A revocation of the chain will only revoke the child tokens unless an event
   for the `audit_id` of the token at the start of the chain is also issued.
   This is done for flexibility in handling revocations. Most cases an event
   for `audit_id` will also be issued if an event for `audit_chain_id` is
   issued.
