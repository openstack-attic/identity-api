...
- *Attribute Mapping Rule*: The user information passed by a federation protocol
...
- *Attribute Map Set*: A set of Attribute Mapping Rules that will be evaluated
  collectively.  A mapping rule may be used in multiple sets, and a set will
...
- *Attribute type mapping Rules:* In these mappings an input Identity API attribute is
  mapped into a Identity API property.

- *Attribute type and value mapping rules:* In these mappings an input Identity
  attribute type and value is mapped into a Identity API property and value.

- *Object mapping rules:* In these mappings a set of federated attributes (types or
  types and values) are mapped into a set of Identity API properties (and values).

API Resources
-------------

### Attribute Mappings: `/OS-MAP/rules`
...
- `requirement` is a set of key terms that may be used to further narrow down
  ...
  following are acceptable values for `requirement`:
    ...
    - `not_any_of` - should return TRUE if none of the specified `values` for
      the specified `type` are presentand otherwise FALSE.


Mapping API
-----------

### Create a set of attribute type and value mapping: `POST /OS-IDP/rules`
...
### Get an attribute type mapping: `GET /OS-IDP/rules/{mapping_id}`

### Update an attribute type mapping: `PATCH /OS-IDP/rules/{mapping_id}`

### List all attribute type mappings: `GET /OS-IDP/attribute-mappings`

### Delete an attribute type mapping: `DELETE /OS-IDP/rules/{mapping_id}`

### Create a set of attribute type and value mapping: `POST /OS-IDP/rules`

{  
    rules:['7fea2d','7fea2e','7fea2e','7fea2f']
}

### Get an attribute map rule set: `GET /OS-IDP/sets/{mapping_id}`

### Update an attribute map rule set: `PATCH /OS-IDP/sets/{mapping_id}`

### List all attribute map rule sets: `GET /OS-IDP/sets`

### Delete an attribute map rule set: `DELETE /OS-IDP/sets/{mapping_id}`
