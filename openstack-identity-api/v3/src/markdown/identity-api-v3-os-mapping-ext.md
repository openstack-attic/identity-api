...
- The `locals` map is a set of Key value pairs.  The key can be one of 
  `username`, `userid`, `group`, `domain`.  The value is a string literal . If it is a string literal, that value will be set directly.  
  TODO(ayoung):  pass through capture groups from the regex?
...
- `values` is an array of limited regular expressions.  The reserved characters are: `*`, `.`, `+`, `()`, `[]` and `\`.  
   `*` matches zero or more characters.
   `.` matches exactly one character
   `+` matches one or more characters
   `\` is used to escape any of the reserved characters so they can be used literally.
