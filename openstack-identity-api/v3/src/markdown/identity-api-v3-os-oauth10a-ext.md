...
extension requires v3.0+ of the Identity API. <<fix me>> An OAuth-derived token will provide
...
- *Request Token:* A token used by the Consumer to obtain authorization from
     ...
     Short lived token - expires in 8 hours.
- *Access Token:* A token used by the Consumer to request new Identity API Service
     ...
     credentials. Long lived token - expires in 60 days.
...
Note that the extra parameter `requested_roles` containing the names of
...
An Access Token is intended to be long lived, and expires in 60 days.
...
from requesting new Identity API service tokens. This also deletes any
Identity API service tokens issued to the Consumer.
