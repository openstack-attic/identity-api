...
a means of impersonating the authorizing user.
...
- *Request Token:* A token used by the Consumer to obtain authorization from
     ...
     Short lived token - expire in 8 hours.
- *Access Token:* A token used by the Consumer to get new Identity API service Tokens on
     ...
     Long lived token - expire in 60 days.
...
A Request Token is intended to be short lived and will expire in 8 hours.
Note that the extra parameter `requested_roles` must also be supplied, these
are the names of the roles the Consumer would like to access.
...
Ac Access Token is intended to be long lived, and expires in 60 days.
...
The Consumer can now request a valid Identity API service tokens which would have the
User's delegated authorization. An OAuth-derived token will provide a means of
impersonating the authorizing user. The generated token's roles will match the
requested roles that the Consumer initially desired.
...
The returned token will be scoped to the authorizing user's project and with the
requested roles. In addition to the usual token response seen in the link below, the
token will also have an OAuth section.
