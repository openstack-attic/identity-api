...
- *Request Token:* A token used by the Consumer to obtain authorization from
     ...
     Short lived token - defaults to expire in 8 hours.
...
By default, a Request Token will expire in 8 hours, however this can
be changed in keystone.conf with the tag `request_token_expiry` under
the `oauth` header. Note that the Request Token begins to expire once it
is created.
