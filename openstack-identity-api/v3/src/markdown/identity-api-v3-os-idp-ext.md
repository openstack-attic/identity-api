...
- `rules` (list of objects)
  ...
  A rule contains both a `local` and a `remote` map.

  - `local` (object)

    References a local Identity API resource, such as a `group` or `user` to
    which the remote attributes will be mapped. The object itself contains a
    single key-value pair, where the key identifies the resource type, and the
    value identifies the local attribute.
