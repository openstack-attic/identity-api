...
- publish Certificates in support of PKI tokens
...
Keystone exports a CA certificate and a signing certificate in order to support PKI token validation.
There are no parameters accepted in the request.  Both certificates are only returned in PEM format.
...
#### Get policy: `GET /certificates/ca
...
#### Get policy: `GET /certificates/signing

What follows is a sample response based on a certificate generated
using openssl.  This is the format produced via `keystone-manage pki_setup.`
Much of the header information is for human consumption only.  The only data
actually required is from::
    -----BEGIN CERTIFICATE-----
to::
    -----END CERTIFICATE-----
