...
authorization on a specific project or domain. Explicit authorization is
expressed as a set of `roles` the specified scope (either a `project` or
`domain`).
...
- `roles` (list of objects)
  ...
  Specifies the authorization available to the token.

  Includes full resource descriptions of each role.
