...
long term shared key needs to be assigned to the party by a cloud administrator.
Assigning a key to a party requires assigning an identity to that party in KDS.
An identity is comprised of a unique party name and the long term shared key
that it is associated with.  This party name is used to identify a party when it
...
When a source party needs to send secure messages to multiple recipients, a
cloud administrator can define a group for those recipients in KDS.  Membership
in a group is determined by comparing a party name with the group name.  If the
name of a party matches `<group name>.*`, it is considered to be a member.  For
...
When a source party requests a ticket where the destination party is a group,
...
only a cloud administrator is allowed to successfully perform the operations.
The authentication and authorization for these requests is left up to the
implementation, though it expected that one would leverage the Identity API for
these purposes.
...
should verify the signature upon receipt of the request before accessing any
data contained in the request metadata.  Failure to verify the signature leaves
the KDS open to issuing a ticket to a party that is impersonating the source.
...
base64 encoded JSON object containing the following key/value pairs:
 ...
 - `skey` - The base64 encoded message signing key.
 - `ekey` - The base64 encoded message encryption key.
