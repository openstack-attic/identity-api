...
communication between arbitrary peers in the system. This allows to
more easily perform centralized access control, prevent unauthorized
communication and avoid the need to perform post authentication access
control and policy lookups on the receiving side.
...
XXX should this request use the signature as resource name ?
...
XXX should this request use the group name as the resource name ?
 ( POST /kds/group_key/scheduler )
...
The requst consist of an owner name and the associated key.
...
XXX should this request use the owner name as the resource name ?
 ( PUT /kds/key/scheduler.host.example.com )

    PUT /kds/key

    {
        "owner": 'scheduler.host.example.com',
...
The requst consist of simply a group name
