...
A Key Distribution Server is used to implement Message Security as definied
...
The Key Distribtion Server (KDS) requires that all ticket requests are
...
#### Ticket Request: `POST /kds/ticket/{signature}`
...
To avoid replay attacks, a timestamp and a nonce are necesary.

A Ticket request is comprises a metadata object and a signature.
...
The Ressponse always returns a triplet of metadata, encrypted ticket and
signature.
...
#### Group Key Request: `POST /kds/group_key/{name:generation}`
...
A group key request is comprises a metadata object and a signature.
...
The Ressponse always returns a triplet of metadata, encrypted group key and
signature.
...
#### Set Key Request: `PUT /kds/key/{owner}`
...
#### Create Group Request: `PUT /kds/group/{group_name}`
...
#### Remove Group Request: `DELETE /kds/group/{group_name}`
