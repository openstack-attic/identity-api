...
- `inherited_by_projects` (boolean)
  ...
  and in the future), rather than on the domain entity itself.  If set to `false`, which
  is the default, then the role is applied only to the entity named in the assignment.
  Assigning a role marked as `inherited_to_projects` to a project directly is simply
