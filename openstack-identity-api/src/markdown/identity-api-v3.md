...
as having private projects, then an `id` or `name` in `domain` must also be
specified to uniquely identify the project. If both a domain and a project
is specified, an HTTP 400 Bad Request will be returned. If neither a project
or domain is provided for scope, the system will use the default project
associated with the user. Otherwise, the system will issue an unscoped token.
...
Optionally, Identity service implementation may return an `authentication`
...
For authentications which require multiple roundtrips, Identity service
implementation may return a 401 Not Authorized with additional information
