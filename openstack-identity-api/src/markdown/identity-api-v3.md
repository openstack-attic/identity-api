...
or `id` in `project` scope) or a domain (by specifiying a `name`
...
implicitly scoped to the Identity service.
...
`id` for the user, then either an `id` or `name` of the user's owning domain
must also be specified as part of the password method.
...
If this user was created in a domain that was specified as having a private
namespace for users and the password method contains `name`, rather than
`id` for the user, then either a `id` or `name` of the user's owning domain
must also be specified as part of the password method.
