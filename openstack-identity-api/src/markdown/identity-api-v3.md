...
`GET /role-assignments?user.id={user_id}` would list all role assignments involving the specified user.
...
`GET /role-assignments?user.id={user_id}&scope.project.id={project_id}&effective` would return the equivilent set of role assignments that would be included in the token response
