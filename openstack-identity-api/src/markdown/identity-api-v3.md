...
`include_catalog` field and it is set to `false` in the authentication
request body.
...
*New in version 3.1*
The caller must present a valid project-scoped token in the
X-Auth-Token header otherwise a HTTP code 401 will be returned.

* Note: a valid project-scoped token is required to harvest the user and
project ids and build an appropriate catalog in response.
