...
- `trust` (object)
  ...
  This object is present in tokens that were created from trust.  A token
...
there is not a way of saying "delegate all roles", in order to prevent users
...
In order to get a list of the trusts for a given trustor, use the filter:
...
This deletes the trust.  Only the trustor will be able to access this URL. Any
other user will get a 403 (Forbidden).
