...
*New in version 3.1* Additionally, if the caller specifies a `include-catalog` boolean in the authentication request body, the authentication response will
either contain or not the services catalog.
...
If the include-catalog is either equal to True or omitted, the authentication
response includes a catalog.
