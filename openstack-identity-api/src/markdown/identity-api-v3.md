...
attributes, whose uniquness are affected by the domain:
  ...
  - *User Name*: This is only unique within a domain.

  - *Project Name*: This is only unique within a domain.

  - *Group Name*: This is only unique within a domain.
...
The first use case is where we want to authorize a user from their credentials,
...
then the domain of the user must also be specfied (by either domain `name` or `id`).
...
an `id` or `name` in `project:domain` must also be specified to uniquely identify the
project. If both a domain and a project are specified, an HTTP 400 Bad Request will
be returned. If neither a project or domain is provided for scope, the system will
use the authenticating user to determine scope.  If the user has a defined default
project (`default_project_id`) attribute then this will be used as the scope.
If there is no default project defined, the scope will be set to the domain that
owns the authenticating user.
...
The authorization scope is processed in exaclty the same way as in the
first user case above.
