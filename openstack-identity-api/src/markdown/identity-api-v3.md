...
Like with individual users, role grants explicitly associate groups with projects 
or domains. A group role grant onto a project/domain is the equivilent of granting
each individual member of the group the role on that project/domain.

Like users, a group entity without any role grants is effectively useless from the 
...
- `domain_id` (string)
  ...
  the client, it will be assigned to the common domain.
...
#### List users who are members of a group: `GET /groups/{user_id}/users`
...
#### Update group: `PATCH /groups/{user_id}`
