...
#### Update role for user on domain: `PATCH /domains/{domain_id}/users/{user_id}/roles/{role_id}`
...
Updating a role assignment can only be used to modify the `inherited` attribute.

Response:

    Status: 204 No Content

#### Update role for group on domain: `PATCH /domains/{domain_id}/groups/{group_id}/roles/{role_id}`
