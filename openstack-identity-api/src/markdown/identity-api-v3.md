...
Role grants explicitly associate users with projects. A user can have the same
role on multiple projects. Granting a user a role on a domain effectively
grants that role across all projects owned by that domain.
