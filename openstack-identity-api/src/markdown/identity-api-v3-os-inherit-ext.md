...
query_filter: group.id, role.id, scope.OS-INHERIT:inherited_to, scope.domain.id,
              scope.project.id, user.id (optional)
...
membership or inherited from the owning domain (if role assignments have been made using
the OS-INHERIT APIs). Such a list will not include the group assigment entities themselves.
If the query_string `effective` is not specified then the API returns a list of the actual
assignments made.
