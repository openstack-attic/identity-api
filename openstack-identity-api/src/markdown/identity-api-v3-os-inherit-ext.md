...
query_filter: domain.id, group.id, OS-INHERIT:inherited_to, project.id, role.id,
              user.id (optional)
...
membership or inheriting from the owning domain. Such a list will not include the group
...
have roles on that entity, and which roles they are. A combined query filter of `user.id`,
`project.id` and `effective` would return the equivilent set of role assignments that would
